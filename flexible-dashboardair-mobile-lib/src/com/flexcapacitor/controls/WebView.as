/**
 * from http://code.google.com/p/flexcapacitor/
 * */
package com.flexcapacitor.controls  {
	...
	public class WebView extends UIComponent {
		 ...
		 * */
		public function set snapshotMode(beInSnapshot:Boolean):void
		{
			//value ? takeSnapshot() : removeSnapshot();
			// sreiner: added check don't do anything if already in right mode
			if (beInSnapshot && !isSnapshotVisible)
			{
				if (webView != null)
				{
					takeSnapshot();
			...
			else if (!beInSnapshot && isSnapshotVisible)
			{
				removeSnapshot();
			}
		}
